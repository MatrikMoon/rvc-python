...
def main():
    ...
    cli_parser.add_argument(
        "-d", "--dir", type=str, help="Directory path containing audio files"
    )
    cli_parser.add_argument(
        "-o",
        "--output",
        type=str,
        default="out.wav",
        help="Output path for single file, or output directory for multiple files",
    )
    cli_parser.add_argument(
        "-mp", "--model", type=str, required=True, help="Path to model file"
    )

    # API parser
    api_parser = subparsers.add_parser("api", help="Start API server")
    api_parser.add_argument(
        "-p", "--port", type=int, default=5050, help="Port number for the API server"
    )
    api_parser.add_argument(
        "-l", "--listen", action="store_true", help="Listen to external connections"
    )
    api_parser.add_argument(
        "-md",
        "--models_dir",
    ...
    api_parser.add_argument(
        "-pm", "--preload-model", type=str, help="Preload model on startup (optional)"
    )

    # Common arguments for both CLI and API
    for subparser in [cli_parser, api_parser]:
        subparser.add_argument(
            "-ip", "--index", type=str, default="", help="Path to index file (optional)"
        )
        subparser.add_argument(
            "-de",
            "--device",
        ...
        subparser.add_argument(
            ...
            type=int,
            default=3,
            help="Apply median filtering to the pitch results",
        )
        subparser.add_argument(
            "-rsr",
            "--resample_sr",
            type=int,
            default=0,
            help="Resample rate for the output audio",
        )
        subparser.add_argument(
            "-rmr",
            "--rms_mix_rate",
        ...
        subparser.add_argument(
            ...
            default=0,
            type=int,
            help="Transpose (integer, number of semitones)",
        )
    ...
    if args.command == "cli":
        ...
        if args.input:
            ...
            print(
                f"Processed {len(output_files)} files. Output directory: {args.output}"
            )
        else:
    ...
    # Handle API command
        ...
        if args.preload_model:
            rvc.load_model(args.preload_model, index_path=args.index)
